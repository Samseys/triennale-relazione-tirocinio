...
\begin{itemize}
  \item \textbf{Flessibilit\`a nel routing dei messaggi} - Rabbit offre molteplici possibilit\`a di configurare gli exchange a seconda delle esigenze:
    \begin{enumerate}
      \item \textit{Direct Exchange} - Invio dei messaggi a code specifiche in base a una routing key
        \begin{figure}[H]
          ...
          \caption{Esempio di Direct Exchange}
        \end{figure}
      \item \textit{Fanout Exchange} - Invio su tutte le code appartenenti all'exchange, utile per il broadcasting
      \item \textit{Topic Exchange} - Permette di instradare i messaggi con routing key basate su pattern pi\`u o meno complessi
        \begin{figure}[H]
          \centering
          \includegraphics[width=7cm]{images/rabbitmq-topic-exchange.png}
          \caption{Esempio di Topic Exchange}
        \end{figure}
  ...
  \item \textbf{Scalabilit\`a} -  Il carico di lavoro viene distribuito automaticamente tra i consumer, e finch\'e i messaggi
    ...
    \vbox{
    \item \textbf{Affidabilit\`a} -  Rabbit \`e in grado di offrire un elevato grado di affidabilit\`a grazie a diversi meccanismi:
      \begin{enumerate}
        \item
