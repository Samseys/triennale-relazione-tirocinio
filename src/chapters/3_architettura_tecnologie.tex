...
\begin{itemize}
  \item \textbf{Comunicazione Asincrona} - Nonostante l'overhead di avere il message broker tra le comunicazioni, questo pattern di comunicazione pu\`o portare a incrementi delle prestazioni
    vista la loro natura asincrona.
  ...
  \item \textbf{Deploy Indipendente} - Al contrario di un sistema monolitico, i servizi possono essere deployati e scalati in maniera indipendente.
    Questo ha notevoli vantaggi: build time pi\`u rapido e possibilit\`a di scalare ogni servizio in maniera indipendente (per esempio aumentando il loro numero)
...
\end{itemize}
Di contro il sistema pu\`o risultare pi\`u difficile da debuggare nella sua totalit\`a, vista la complessit\`a aumentata e la quantit\`a di parti.
